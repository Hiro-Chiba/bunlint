...
- ディレクトリ：`app/`を利用。以下のページ・コンポーネントを想定。
  ...
  - `components/StatsPanel.tsx`：文字数/内容語数/文章数などの統計情報表示（実装済み）。
...
- 入力テキストが変化するたびに即時更新。
- 文字数、内容語数（助詞などの機能語を除外）、文数（「。」「．」「!」「?」等で分割）を表示。
- 現状の内容語数計算ロジックに不具合があるため、スペースや改行を考慮した堅牢な再実装を行い、ユニットテストを追加して再発防止を図る（未実装）。

- 統計カードの「内容語数」をクリックすると、エディタ内の各語に背景色（例：緑）を付与してハイライトし、連続する語はスペース部分にも薄い背景色を適用して区切りが視認できるようにする。
- 「文数」をクリックした場合は単語ハイライトを解除し、代わりに文ごとに背景色を付与して区切りを明示する。単語ハイライトと文ハイライトは同時に有効化されないようトグル制御を実装する。
- 「文字数」は表示のみで追加のアクションは不要とし、クリックしても状態変化が発生しないようにする。
...
- Gemini 2.0 Flash-Lite で利用制限やスロット不足によりエラーが発生した場合は Gemini 2.0 Flash へ自動的に切り替え、ユーザーへ追加操作を求めない。
- モデル切替の実装では 429 / 503 系レスポンスや `quota` / `exhausted` などのエラー文言を検知してフォールバックを判断し、冗長な再試行や過剰なログ出力を防ぐ。
...
3. 文字数カウント・句読点変換ロジック
   - 文字数/内容語数計算のユーティリティ作成（`lib/text.ts`、実装済み）。
   - 内容語数カウントロジックの不具合を修正し、境界条件に対応したテストを追加する（未実装）。
...
- 特別な暗号を入力することで 10 分間だけ Gemini 3 Pro Preview を利用できる「高精度モード」を提供する。暗号は環境変数`GEMINI_HIGH_ACCURACY_CODE`で管理し、成功時はサーバー側で署名付きクッキーを発行してリクエストごとに判定する。
- フロントエンドには「AIの精度を上げる」ボタンを設置し、モーダルで暗号の入力とバリデーション結果（成功／失敗）を表示する。期限切れはクライアント側で自動的に検知し、状態をリセットして利用者にフィードバックする。
- サーバー側の語尾変換 API は高精度モードの有効期限を確認し、アクティブな間のみ Gemini 3 Pro Preview を優先利用しつつ従来モデルへのフォールバックも維持する。
