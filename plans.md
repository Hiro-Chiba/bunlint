...
- 特別な暗号を入力することで 10 分間だけ Gemini 2.5 Flash を利用できる「高精度モード」を提供する。暗号は環境変数`GEMINI_HIGH_ACCURACY_CODE`で管理し、成功時はサーバー側で署名付きクッキーを発行してリクエストごとに判定する。
- フロントエンドには「AIの精度を上げる」ボタンを設置し、モーダルで暗号の入力とバリデーション結果（成功／失敗）を表示する。期限切れはクライアント側で自動的に検知し、状態をリセットして利用者にフィードバックする。
- サーバー側の語尾変換 API は高精度モードの有効期限を確認し、アクティブな間のみ Gemini 2.5 Flash を優先利用しつつ従来モデルへのフォールバックも維持する。
